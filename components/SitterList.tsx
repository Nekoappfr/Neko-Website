      ...
      <div className="horizontal-scroll-container no-scrollbar py-2 gap-4 md:gap-8 lg:gap-10">
        {MOCK_SITTERS.map((sitter) => (
          ...
          >
            ...
            <div className="relative w-full h-[180px] md:h-[260px] rounded-[24px] md:rounded-[40px] overflow-hidden shadow-sm hover:shadow-md transition-all duration-500 bg-[#F0F0EF] mb-3">
              <img 
                src={sitter.image} 
              ...
              <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 z-10 flex items-center justify-between gap-1.5">
                <h3 className="text-[18px] md:text-[24px] font-bold text-white leading-tight tracking-tighter truncate flex-1">
                  {sitter.name}
                </h3>
                <div className="flex items-center gap-1 bg-white/20 backdrop-blur-md px-2 py-0.5 rounded-lg flex-shrink-0 border border-white/20">
                  <StarIcon />
                  <span className="text-[10px] md:text-[11px] font-bold text-white">{sitter.rating}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col px-1 space-y-2">
              <div>
                <div className="flex items-center justify-between gap-1 mb-0.5">
                  <span className="text-[#1C1C1B] text-[13px] md:text-[15px] font-bold tracking-tight flex items-center gap-1 truncate">
                    <PinIcon /> {sitter.location}
                  </span>
                  <span className="text-[#1C1C1B] text-[12px] md:text-[13px] font-bold">
                    {sitter.priceFrom}€
                  </span>
                </div>
                <p className="text-[#37352F]/60 text-[11px] md:text-[12px] font-medium leading-tight italic line-clamp-2">
                  "{sitter.review.text}"
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-[#F3F3F2]">
                ...
                <span className="text-[10px] md:text-[11px] font-bold text-[#37352F]/40 uppercase tracking-wider">
